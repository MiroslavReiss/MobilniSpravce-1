...
switch ($_SERVER['REQUEST_METHOD']) {
    ...
    case 'POST':
        ...
        $stmt = $pdo->prepare('INSERT INTO todos (title, user_id) VALUES (?, ?) RETURNING id, title, completed');
    ...
    case 'PATCH':
        ...
        if (isset($data['completed'])) {
            $stmt = $pdo->prepare('UPDATE todos SET completed = ? WHERE id = ? RETURNING *');
        ...
        if (isset($data['title']) && $_SESSION['username'] === 'madkoala') {
            $stmt = $pdo->prepare('UPDATE todos SET title = ? WHERE id = ? RETURNING *');
